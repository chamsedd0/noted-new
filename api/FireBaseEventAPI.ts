...
async function deleteCourseEvents(userId: string, courseId: string): Promise<void> {
  try {
    const userRef = getUserRef(userId);
    const events = await getUserEvents(userId);
    await userRef.update({
      events: events.filter((e: Event) => !(e.type === "course" && e.courseId === courseId))
    });
  } catch (error) {
    throw new Error((error as Error).message);
  }
...
async function updateCourseEvents(
  ...
  try {
    ...
    const events = await getUserEvents(userId);
    const updatedEvents = [
      ...events.filter((e: Event) => !(e.type === "course" && e.courseId === courseId)),
      ...newEvents
    ];
